...
- **Favorites layout, your choice:** Settings → Display now offers Auto, one column, or two columns for the Favorites list — Auto keeps today's width behavior.
- **Easier to hit, calmer with animations off:** icon buttons across arrivals, lists, search, and the trip planner now meet the full touch-target size, and turning on Remove animations also stills press feedback, list movement, countdown flips, map camera glides, and loading shimmer.
...
- **Save stops from anywhere you find them:** search results, recent stops, and the lines browser now show a heart on each stop that you can tap to save or unsave it. It fills in when a stop is saved, so you can always see at a glance what is in your favorites. The same heart now replaces the trash icon in your favorites list and the star in Recents. Removing a favorite from your list still asks you to confirm first — but it no longer offers an Undo afterwards.
...
- **Nearby stops now tell you how far and which way:** each row in the nearby list shows how far away the stop is and the compass direction to it, nearest first. Distances come from TriMet; the direction is worked out from where you are. Lists that do not know your location are unchanged.
- **Arrivals holds up better:** a scheduled time no longer looks like a live one, and the stop number is easier to read.
- **Smaller craft fixes across the lists:** stop names and line badges survive a large font setting without clipping, searching shows the same shimmering placeholders as every other list, the corner-radius slider previews as you drag, and reordering your favorites now explains itself once.
- **One heart everywhere:** the heart on the arrivals screen is now the same one used on search results, Recents, Lines and Favorites.
- **One vocabulary:** the app now calls a transit line a "line" everywhere (it previously mixed "route" and "line"), the heart always "saves", and the Recents screen uses the same name as its tab.
- **The app no longer crashes on a background schedule refresh:** it was downloading TriMet's whole schedule file and parsing a 44 MB table of route shapes that nothing in the app uses. That alone needed more memory than the app is allowed, so the refresh never once completed — and because the failure was caught, it left the app running out of memory, free to be killed at any moment (which is why it could die while you were in Settings or looking at nearby stops). That table is no longer read, and the refresh now completes.
...
- **Fix: silence FlowPreview and unnecessary safe-call warnings in release build.**
...
- **Denser lists on wide screens:** when the window is wide enough, favorites, recent stops, stop search results, and the route browser flow into two tidy columns instead of one stretched list.
...
- **Departure alerts (opt-in):** get a notification when the next bus, train, or MAX at a stop you choose is about to leave. Turn "Departure alerts" on in Settings, pick the favorite stops to monitor, and choose to be pinged 5, 10, or 15 minutes before departure. Alerts pause overnight (11 PM–5 AM), and each departure notifies you once. Everything is off by default.
- **Offline stop search:** the latest stop list is cached on-device, so looking up a stop keeps working without a connection (e.g., in a tunnel or on airplane mode). Stops refresh more often while online, and the cached copy is only ever replaced by a newer, complete network list.
...
- **Fixed:** the Trip Planner stopped working whenever TriMet answered — Android's XML parser rejected a security-hardening flag, so every itinerary request failed with "Trip Planner is temporarily unavailable" even though the service responded correctly. Plans now load as before. The Trip Planner also now tells real TriMet outages apart from connectivity hiccups — a dropped network shows "Can't reach TriMet right now" and retries once, reserving "temporarily unavailable" for genuine server trouble.
