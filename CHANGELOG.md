...
- **Smaller craft fixes across the lists:** stop names and route badges survive a large font setting without clipping, searching shows the same shimmering placeholders as every other list, the corner-radius slider previews as you drag, and reordering your favorites now explains itself once.
- **One heart everywhere:** the heart on the arrivals screen is now the same one used on search results, Recent stops, Lines and Favorites.
- **One vocabulary:** the app now calls a transit line a "line" everywhere (it previously mixed "route" and "line"), the heart always "saves", and the Recent stops screen uses the same name as its tab.
...
- **Trip planner asks first:** it no longer asks for your location just because you opened the page, each endpoint row can pick a spot on the map directly, an arrive-by time in the past is refused rather than sent, and the time picker follows your phone's 12- or 24-hour setting.
