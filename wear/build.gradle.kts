...
abstract class RenameWearApkTask : DefaultTask() {
    @get:Internal
    ...
    fun renameApk() {
        ...
        builtArtifacts!!.elements.forEach { artifact ->
            val newName = "PdxBusTracker-wear-${variantName.get()}-${artifact.versionName}.apk"
            Files.copy(
...
android {
    ...
    signingConfigs {
        create("release") {
            storeFile = file(rootDir.absolutePath + "/app/" + (project.findProperty("STORE_FILE") ?: "release.keystore"))
...
dependencies {
    ...
    // Stand-alone watch tile ("Next departure") with background refresh
