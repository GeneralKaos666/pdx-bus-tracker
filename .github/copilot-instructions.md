...
There are no ViewModels, DI framework, or Room. Screens keep UI state with `remember`/`mutableStateOf` and call suspend repository APIs. Keep raw `TransitApi` and `DatabaseHelper` calls behind repository interfaces, including in widget workers.
...
For release builds, signing credentials are supplied with `STORE_PASSWORD`, `KEY_ALIAS`, and `KEY_PASSWORD`, or use `-PreleaseSigningFallback=true` only for a local smoke test. Phone release version codes can be resolved from Play by the Gradle Play Publisher plugin when credentials are configured.
...
- Shared arrival behavior belongs in `common/model` helpers. Preserve `arrivalKey`/`dedupeArrivals` so phone and widget lists cannot produce duplicate keys.
...
When user-facing text names the TriMet or TransitTracker marks, include the existing trademark attribution string from the phone string resources:
